    ...
    <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
      ...
      <nav className="flex-1 p-4">
        <ul className="space-y-2">
          {navigationItems
            .filter((item) => {
              if (role === "admin") return item.href !== "/";
              if (role === "user") return item.href === "/";
              return false;
            })
            .map((item) => (
              <li key={item.label}>
      ...
      <div className="p-4 border-t border-gray-200">
        <button
          onClick={() => logout()}
          className="flex items-center cursor-pointer gap-3 w-full px-4 py-3 text-red-500 font-medium text-xs hover:bg-red-50 rounded-lg transition-all"
        >
